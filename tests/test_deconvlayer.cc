...
{
  /* Loads data */
  int width = 24, height = 40, channel = 21;
  Image<float> input(width, height, channel, 1);
  for (int k = 0; k < channel; k++) {
    ifstream infile("./outputs/channel" + to_string(k) + ".txt");
    for (int j = 0; j < height; j++) {
      for (int i = 0; i < width; i++) {
        infile >> input(i, j, k, 0) >> ws;
      }
    }
    infile.close();
  }
  ...
  LayerParameter layer = net_model->layer(40);
  ...
  Func output_func = deconv_layer.run(input_func, width, height, channel, 1);
  ...
  int output_num = deconv_layer.get_num();
